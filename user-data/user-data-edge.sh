...
cp -av /tmp/user-data/edge/nginx/nginx.conf /etc/nginx/
cp -av /tmp/user-data/edge/awslogs/awslogs.conf /etc/awslogs/
...
sed -i "s|%BACKUPORIGINPRIVATEIP%|$BACKUPORIGINPRIVATEIP|g" /etc/nginx/nginx.conf
